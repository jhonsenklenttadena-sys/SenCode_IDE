...
export function ChatPanel({
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    ...
    if (!imageItem) return; // not an image — let default paste happen
    e.preventDefault();
  ...
  useEffect(() => {
    ...
    if (isStreaming) return;                       // wait until stream done
    const completed = messages.filter((m) => m.role === 'assistant' && !m.streaming && !m.error);
    if (completed.length === 0) return;           // nothing to save yet
  ...
  // No token/context limit — students have full access to all their files.
  ...
  const handleSend = useCallback(async (overrideInput?: string) => {
    ...
    // ── Build context from open editor tabs (already in memory, no disk read) ──
    // This avoids flooding the context with every file in the project.
    // The student controls what's in context by what they have open.
    let contextBlock = '';
    ...
    // ── /debug-project: read all project files from disk ─────────────────
    let debugBlock = '';
    ...
    // Build folder structure summary so AI always knows what files exist
    ...
    // Base prompt (editable in Settings for now) + user's custom
    // instructions (if any) + live workspace/session context.
    const systemContent = composeSystemPrompt(settings.basePrompt, settings.customInstructions, dynamicContext);

    // Include full conversation history so the model has context across turns.
    // `messages` still reflects state before the current userMsg was appended
    // (React batches the setMessages call above), so we add userMsg explicitly at the end.
    const chatMessages = [
  ...
  const downloadAllGenerated = () => {
    // In the desktop build this uses archiver to create a real zip.
    // Here we download a single concatenated file as a fallback.
